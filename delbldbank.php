...
<body>
<form action ="delbldbank.php" method= "post">
<div class="container">
<header>
	...
	<?php
		...
		$collection= $db->bankdetail;
			...
			foreach($cursor as $document){
				$data .= "<tr>";
				$data .= "<td>" . $document["bankname"] . "</td>";
				$data .= "<td><input type='checkbox' name='banknm[]' value='".$document["bankname"]."'></td>";
				$data .= "</tr>";
			}
			...
			$data .= "</table>";
			echo $data;
			if(!empty($_POST['banknm']) && isset($_POST['submit'])) {
			foreach($_POST['banknm'] as $check) {
					$collection->remove(array('bankname'=>$check)); //echoes the value set in the HTML form for each checked checkbox.
